...
public class PawnInteractionTest {
	...
	public void testEvaluateFighting() {
		...
		assertTrue("C'est une égalité.", fourthFight.evaluateFighting() == 0);
		...
		assertTrue("Le pion au rang 1 a gagné le combat.", fifthFight.evaluateFighting() == 1);
	}
	 ...
	 assertTrue("Le pion au rang 9 a gagné le combat", (squareA.getPawn() == null) && (squareB.getPawn() == pawnA));
	
	 // Ici, un rang 0 contre un rang 8, le 8 doit être le gagnant.
	 PawnInteraction secondFight = new PawnInteraction(squareC, squareB, grid);
	 secondFight.doFighting();
	 //assertTrue("Le pion au rang 8 a gagné le combat.", (squareB.getPawn() == null) && (squareC.getPawn() == pawnB));
	 
	// Ici, un rang 0 contre un rang 9, le 0 doit être le gagnant.
	PawnInteraction thirdFight = new PawnInteraction(squareC, squareA, grid);
	thirdFight.doFighting();
	assertTrue("Le pion au rang 0 a gagné le combat.", (squareC.getPawn() == null) && (squareC.getPawn() == pawnA));
	 }
